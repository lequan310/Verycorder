...
import { getReplayBoundingBox, getReplayTargetBBox } from "./openai";
...
export async function initBBox() {
  ...
  fs.writeFileSync("./logs/bboxes.json", JSON.stringify(bboxes, null, '\t'));
  fs.writeFileSync("./logs/bboxes.png", drawnBuffer);
  return bboxes;
}
...
export async function getViewScreenshotBuffer() {
  ...
  async () => {
    try {
      ...
      fs.writeFileSync("./logs/screenshot.png", screenshot);
    } catch (_) {
    }
  }
