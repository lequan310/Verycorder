...
import { handleClickRecord, handleClickReplay, handleNavigate, handleNavigateInPage, handleTestCaseEnded, handleUpdateTestCase, updateTestSteps, ipcGetMode, testLogEvents } from "./ipcFunctions";
...
export function toggleRecord() {
  if (currentMode === AppMode.replay || currentMode === AppMode.disabled) return;
  ...
  return new Promise<ChangeUrlResult>((resolve, reject) => {
    ...
    view.webContents
      ...
      .then(() => {
        ...
        if (currentMode === AppMode.disabled) currentMode = AppMode.normal;
      })
      .catch((error) => {
        signal.removeEventListener("abort", handleAbort);
...
export async function toggleReplay() {
  if (currentMode === AppMode.record || currentMode === AppMode.disabled) return;
  ...
  if (currentMode === AppMode.replay && testCase && testCase.events && testCase.events.length > 0) {
